...
var assets = {
  source_app: _.map(require(paths.build + 'gulp-' + target + '-assets-' + mode + '.json'), function (o) {
    return o.replace(/\{\{(.+?)\}\}/g, target)
  }),
  dest_develop: paths.pub_client_develop + 'vendor/',
  dest_production: paths.pub_client_production + 'vendor/'
...
// ASSETS
// copy file from asset folder into the app vendor folder
gulp.task('assets:app', function() {
  log('Copying assets app..');

  var jsFilter = $plugins.filter('**/*.js', {restore: true});
  var cssFilter = $plugins.filter('**/*.css', {restore: true});

  return gulp.src(assets.source_app, {base: paths.assets})
    .pipe($plugins.expectFile(assets.source_app))
    .pipe(jsFilter)
    // .pipe($plugins.if(isProduction, $plugins.uglify(vendorUglifyOpts)))
    .pipe(jsFilter.restore)
    .pipe(cssFilter)
    // .pipe($plugins.if(isProduction, $plugins.minifyCss()))
    .pipe(cssFilter.restore)
    .pipe(gulp.dest(isProduction ? assets.dest_production : assets.dest_develop))
});

gulp.task('assets', gulpsync.sync(['assets:app']) );
