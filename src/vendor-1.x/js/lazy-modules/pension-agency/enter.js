/**
 * enter Created by zppro on 17-2-27.
...
(function() {
    ...
    angular
        ...
        .controller('EnterGridController', EnterGridController)
        .controller('EnterDetailsController', EnterDetailsController)
    ;


    EnterGridController.$inject = ['$scope', 'ngDialog', 'vmh', 'entryVM'];

    function EnterGridController($scope, ngDialog, vmh, vm) {
        ...
        function disableEnterRelatedAction(row){
            return vmh.psnService.disableEnterRelatedAction(row._id);
        }
        ...
        function completeEnter(row){
            ngDialog.openConfirm({
                ...
                var promise = vmh.psnService.completeEnter(row._id, {
    ...
    function EnterDetailsController($scope, ngDialog,PENSION_AGENCY_DEFAULT_CHARGE_STANDARD, PENSION_AGENCY_CHARGE_ITEM, vmh, vm) {
        ...
        function init() {
            ...
            vmh.parallel([
                ...
                if (vm.selectedStandard) {
                    ...
                    //伙食信息
                    console.log(vm.selectedStandard.children);

                    var board_charge_item = _.findWhere(vm.selectedStandard.children, {_id: PENSION_AGENCY_CHARGE_ITEM.BOARD + '-' + vm.elderlyModel.charge_standard});
                    if (board_charge_item) {
                        console.log(board_charge_item)
        ...
        function disableEnterRelatedAction(model){
            return vmh.psnService.disableEnterRelatedAction(model._id).then(function(){
                return true;
            });
        }
        ...
        function completeEnter(){
            ngDialog.openConfirm({
                ...
                var promise = vmh.psnService.completeEnter(vm.model._id, {
                    operated_by: vm.operated_by,
                    operated_by_name: vm.operated_by_name
                });
