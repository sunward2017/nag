  ...
  function DrugOutStockDetailsController($scope, ngDialog, vmh, vm) {
    ...
    function importFromDrugUseItem() {
      ...
      vm.modelNode.services['psn-drugUseItem'].query({
        ...
        var repeat_value, drug_name, drug_mini_unit, drug_quantity;
        var grouped = _.groupBy(rows, function(o){
          return o.drugId.id
        });
        for (var drugId in grouped) {
          var groupRows = grouped[drugId];
          if (groupRows.length>0) {
            drug_name = groupRows[0].drugId.short_name || groupRows[0].drugId.full_name;
            drug_mini_unit = groupRows[0].drugId.mini_unit
            drug_quantity = 0;
            _.each(groupRows, function (o) {
              repeat_value = o.repeat_values.length > 1 ? o.repeat_values.length : 1;
              console.log('repeat_value:', repeat_value);
              drug_quantity +=  repeat_value * o.quantity
              console.log(drug_name, drug_quantity)
            });
            syncToDrugListWhenAdd({drugId: drugId, drug_name: drug_name, mini_unit: drug_mini_unit, quantity: drug_quantity});
          }
        }
      });
