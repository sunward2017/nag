...
module.exports = {
  ...
  psn$drugDirectory$$name: function (id) {
    var self = this;
    return co(function *() {
      try {
        var drug = yield self.ctx.modelFactory().model_read(self.ctx.models['psn_drugDirectory'], id)
        if (drug.full_name) {
          yield self.ctx.modelFactory().model_bulkUpdate(self.ctx.models['psn_drugStock'], {
            batchModel: {drug_name: drug.full_name},
            conditions: {
          ...
          yield self.ctx.modelFactory().model_bulkUpdate(self.ctx.models['psn_drugUseItem'], {
            batchModel: {name: drug.full_name},
            conditions: {
              tenantId: drug.tenantId,
              status: 1,
              drugId: drug._id
          ...
          var drugInOutStocks = yield self.ctx.modelFactory().model_query(self.ctx.models['psn_drugInOutStock'], {
            select: 'drugs',
            where: {
              tenantId: drug.tenantId,
              status: 1,
              'drugs.drugId': drug._id
            }
          });
          var drugInOutStock, drugsInOutStock;
          for (var i = 0, len = drugInOutStocks.length; i < len; i++) {
            // 查找老人
            drugInOutStock = drugInOutStocks[i];
            for (var j = 0, lenj = drugInOutStock.drugs.length; j < lenj; j++) {
              if(drugInOutStock.drugs[j].drugId.toString() == drug._id.toString()){
                drugInOutStock.drugs[j].drug_name = drug.full_name
              }
            }
