...
module.exports = function(ctx,name) {
    if (module.isloaded) {
        ...
        var mealMenuSchema = new mongoose.Schema({
        ...
        mealMenuSchema.virtual('x_axis_value').get(function () {
            return ctx.moment(this.x_axis).day();
        });

        mealMenuSchema.virtual('y_axis_value').get(function () {
            if (this.y_axis) {
        ...
        mealMenuSchema.pre('update', function (next) {
            this.update({}, {$set: {operated_on: new Date()}});
            next();
        });

        return mongoose.model(name, mealMenuSchema, name);
    }
}